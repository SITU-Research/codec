...
exports.handler = async (event, context, callback) => {
  try {
    ...
    const requested_sheet_offset =
      parseInt(event.queryStringParameters.offset) - 2;

    let sheet_ids_by_title = {};
    Object.keys(doc._rawSheets).forEach((key) => {
      let sheet = doc._rawSheets[key];
      sheet_ids_by_title[sheet._rawProperties.title] =
        sheet._rawProperties.index;
    });

    if (requested_sheet_title === "platformconfig") {
      let platform_config_rows = await doc.sheetsByIndex[
        sheet_ids_by_title["Platform config"]
      ].getRows({ offset: requested_sheet_offset });
      let platform_config = {};
      platform_config_rows.forEach((platform_config_row) => {
        platform_config[platform_config_row._rawData[0]] =
          platform_config_row._rawData[1];
      });
      return {
        statusCode: 200,
